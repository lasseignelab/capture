...
## cap_md5_verify
Verifies that data generated by a script is reproduced with the exact same
output. This function is used in verification files stored in the verifications
directory within each project.
```
cap_md5_verify FILE
```
- `FILE` The verification file to execute.
...
Example:
```
cap_md5_verify --select "*/raw_feature_bc_matrix/*" "$CAP_DATA_PATH/cell_ranger"
```
